...
function tryParseJSON(jsonString, pageNumber, job_id, attemptType) {
    if (!jsonString) {
    ...
    try {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            if (combinedResponse.ok) {
                const combinedResultJson = await combinedResponse.json();
            ...
            if (!attemptSuccess) {
                ...
                const extractParts = [{ inlineData: { mimeType: "image/png", data: pageImageBase64.split(',')[1] } }];
                ...
                let extractedTextForFallback = null;
                if (extractResponse.ok) {
                    const extractResultJson = await extractResponse.json();
                    ...
                    if (parsedExtract && parsedExtract.extracted_text && typeof parsedExtract.extracted_text === 'string') {
                        ...
                        pageNotes += ` | FB-Extract ParseFAIL (${extractResponseTime}ms). Raw: ${extractStructResponse ? extractStructResponse.substring(0, 100) + '...' : 'NO_RESPONSE_TEXT'}`;
                    }
                }
                else {
                ...
                if (extractedTextForFallback !== null && extractedTextForFallback.trim().length > 0) {
                    ...
                    const textToUseInPrompt = extractedTextForFallback;
                    const translateParts = [{ text: `Translate the following text accurately to ${targetLangName}. Preserve the original meaning and structure where possible. Text to translate: ###${textToUseInPrompt}###` }];
                    ...
                    if (translateResponse.ok) {
                        const translateResultJson = await translateResponse.json();
                        ...
                        if (parsedTranslate && parsedTranslate.translated_text) {
                            ...
                            pageNotes += ` | FB-Translate ParseFAIL (${translateResponseTime}ms). Raw: ${translateStructResponse ? translateStructResponse.substring(0, 100) + '...' : 'NO_RESPONSE_TEXT'}`;
                        }
                    }
                    else {
            ...
            if (!attemptSuccess) {
                ...
                if (finalExtractedText === "[Image text to be extracted by Gemini]" && imageOnly)
                    finalExtractedText = "[Image, text extraction failed]";
            }
            const pageResult = {
        ...
        if (downloadSuccess) { // Only attempt to unlink if download was successful and file was created
            fs.unlink(workerTempFilePath, (err) => {
                if (err) {
        ...
        if (downloadSuccess && fs.existsSync(workerTempFilePath)) { // <<< Use workerTempFilePath
            fs.unlink(workerTempFilePath, (unlinkErr) => {
                if (unlinkErr)
                    console.warn(`⚠️ Failed to delete worker's temp file ${workerTempFilePath} during error handling:`, unlinkErr);
                else
                    console.log(`🗑️ Deleted worker's temp file ${workerTempFilePath} during error handling.`);
            });
        }
